"""To solve the single-level optimization of congestion game (on di-graphs)
by convex optimizaion using JuMP."""
...
if specified_toll
    ...
    seed_alg = 100                      ## seed for MP algorithm
    ...
    model = Model(optimizer_with_attributes(Ipopt.Optimizer))
    @variable(model, xv[1:no_of_class*no_of_edges] >= 0.)
    xn_values, runtime = cal_opt_flow!(model, xv, B, s, q, d, cpc, tf, Λ, τ, level="lower")
    Φn_wτ, Hn_wτ = Φ_and_H(transpose(xn_values) * ones(no_of_class), cpc, tf, s, q, zeros(no_of_edges), edge_list)
end
